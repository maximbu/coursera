...
public class ModelBL {
   ...
   public int getNumOfFacilities() {
	...
	public void flipFacilityEnabled(int facilityId)
	{
		setFacilityEnabled(facilityId,!model.getFacility(facilityId).isEnbled());
	}
	...
	public void assignPerRequested(int[] assignedFacilities) {
		for(int i=0;i<model.getNumOfFacilities();i++)
		{
			setFacilityEnabled(i, false);
		}
		for(int i=0;i<model.getNumOfCustomers();i++)
		{
			tryAssignCustomerToFacility(i, assignedFacilities[i], true);
		}
	}
	...
	public boolean allCustomersAssigned() {
		for(int i=0;i<model.getNumOfCustomers();i++)
		{
			if(model.getCustomer(i).getAssignedFacility() == -1)
				return false;
		}
		return true;
	}
