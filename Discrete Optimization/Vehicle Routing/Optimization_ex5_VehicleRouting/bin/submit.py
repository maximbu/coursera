...
def basic_prompt():
    """Prompt the user for login credentials. Returns a tuple (login, password)."""
    login = 'maximbu@gmail.com' #raw_input('Login (Email address): ')
    password = 'dgufjEBNwd' #raw_input('Submission Password (from the programming assignments page. This is NOT your own account\'s password): ')
    return (login, password)
...
def output(part):
    ...
    start = time.clock()
    try:
        print part.input_file
        print os.path.basename(part.input_file)
        solution = solve_it(load_input_data(part.input_file),os.path.basename(part.input_file))
    except Exception, e:
